...
srs_error_t SrsDispatch::init()
{
    ...
    if ((err = init_shm(log_queue_, PARSER_SHARE_KEY, PARSER_QUEUE_MEM_SIZE)) != srs_success) {
...
srs_error_t SrsDispatch::init_shm(SrsCircleQueue *queue, int shm_key, int shm_size)
{
    srs_error_t err = srs_success;
    if (queue == NULL) {
        return srs_error_new(ERROR_QUEUE_INIT, "queue is not created");
    }
    err = queue->init(shm_key, shm_size);
    if (err != srs_success) {
...
srs_error_t SrsDispatch::dispatch_log(SrsStWork &work)
{
    ...
    unsigned int work_num = log_queue_->get_curr_size();
...
srs_error_t SrsDispatch::fetch_log(SrsStWork &work)
{
    ...
    char result[1024] = {0};
    uint64_t size = 0;
    err = log_queue_->pop(result, size);
